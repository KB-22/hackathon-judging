...
- On a host (Render, Railway, Fly, a VPS) use `DB_DRIVER=postgres` with `SUPABASE_DB_URL`, so the app is stateless and can restart or scale without losing data. `GET /healthz` is a readiness probe that checks the database.
...
tests/             scoring.test.js, driver.test.js, postgres.test.js (real Postgres via PGlite)
```
